          ...
          <Modal.Body>
            <HStack marginBottom={4}>
              ...
              <BaseInput
                ...
                marginLeft={2} placeholder="Nhập địa điểm, tên khu nghỉ dưỡng" flex={1}
                fontSize={16}
                value={search}
                onChangeText={(text) => {setSearch(text)}}
                onSubmitEditing={(e) => {handleSearch(e.nativeEvent?.text)}}
              ></BaseInput>
              <Pressable onPress={() => selectLocation(search)}>
                <Icon as={Ionicons} size={10} name='checkmark-outline'></Icon>
              </Pressable>
            </HStack>
            <Divider></Divider>
            ...
            <ScrollView>
              {
                locations.map(location => {
                  return (
                    <Pressable onPress={() => selectLocation(location.location_name)} key={location.location_id}>
